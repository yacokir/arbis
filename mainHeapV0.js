...
function testaArbitragens() {
  ...
  if (vendeDomesticamente > gatilho || compraDomesticamente > gatilho) {
    ...
    if (csvLines.length > 0) {
      ...
      outputFile.push(JSON.stringify(inconsistencies.slice(-50), null, 2)); // Limita a 50 inconsistências por snapshot
      outputFile.push(separator);
...
const intervalId = setInterval(() => {
  ...
  const snapshotOutput = [
    ...
    snapshotMetrics,
    'INCONSISTENCIES:',
    JSON.stringify(inconsistencies.slice(-50), null, 2), // Limita a 50 inconsistências por snapshot
    '===================='
  ].join('\n') + '\n';
