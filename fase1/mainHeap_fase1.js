...
const coordenador = new Worker('./coordenador_fase1.js', {
  workerData: { port: coordToMain },
  transferList: [coordToMain]
});
coordenador.on('error', (err) => {
  log(`Erro no coordenador: ${err.message} <==================================`);
});
coordenador.on('exit', (code) => {
  log(`Coordenador terminou com código ${code} <==================================`);
});
mainToCoord.on('message', (msg) => {
  if (msg.type === 'pong') {
    log(`Pong recebido do coordenador <==================================`);
  }
});
mainToCoord.postMessage({ type: 'ping' });
log('MainHeap Fase 1 iniciado, ping enviado ao coordenador <==================================');
