...
// Adições para Fase 2: Comunicação com matriz simulada
const logFile = path.join(__dirname, '..', 'saida.txt');
const phaseLogDir = 'C:\\Users\\Yaco\\Desktop\\Cryptos\\ARBIS\\logs';
const phaseLogFile = path.join(phaseLogDir, 'fase2.log');
...
const { port1: mainToCoord, port2: coordToMain } = new MessageChannel();
const coordenador = new Worker('./coordenador_fase2.js', {
  workerData: { port: coordToMain },
  transferList: [coordToMain]
});
...
// Matriz simulada para teste (mantendo amount: 0 na T1)
const mockMatrix3 = [
...
mainToCoord.on('message', (msg) => {
  if (msg.type === 'pong') {
    log(`Pong recebido do coordenador <==================================`, true);
    // Envia matrizes simuladas após pong
    setTimeout(() => {
      mainToCoord.postMessage({ type: 'executeArbitrage', operacoes: mockMatrix3, t0: Date.now() });
      log(`Enviada matriz simulada com 3 ordens: ${JSON.stringify(mockMatrix3)} <==================================`, true);
    }, 1000);
    setTimeout(() => {
      mainToCoord.postMessage({ type: 'executeArbitrage', operacoes: mockMatrix2, t0: Date.now() });
...
mainToCoord.postMessage({ type: 'ping' });
log('MainHeap Fase 2 iniciado, ping enviado ao coordenador <==================================', true);
