...
function construirMatriz(vendeDomesticamente, compraDomesticamente) {
  const matriz = [];
  ...
  if (vendeDomesticamente > gatilho) {
    ...
    matriz.push({
      ...
      side: 'buy',
      type: 'market',
      amount: bests[dolar + moeda].ask.amount.toString(),
      price: null,
      timeInForce: null
    });
    matriz.push({
      ...
      side: 'buy',
      type: 'market',
      amount: bests[cripto + dolar].ask.amount.toString(),
      price: null,
      timeInForce: null
    });
    ...
    matriz.push({
      ...
      type: 'limit',
      amount: '0', // Definido como 0 para rejeição
      price: bests[cripto + moeda].ask.preco.toString(),
      timeInForce: 'IOC'
    });
    matriz.push({
      ...
      side: 'sell',
      type: 'market',
      amount: bests[dolar + moeda].bid.amount.toString(),
      price: null,
      timeInForce: null
    });
    matriz.push({
      ...
      side: 'sell',
      type: 'market',
      amount: bests[cripto + dolar].bid.amount.toString(),
      price: null,
      timeInForce: null
    });
  }
...
function testaArbitragens() {
  ...
  if (vendeDomesticamente > gatilho || compraDomesticamente > gatilho) {
    ...
    if (csvLines.length > 0) {
      ...
      // Chamar construirMatriz e enviar ao coordenador
      const matrizReal = construirMatriz(vendeDomesticamente, compraDomesticamente);
...
const coordenador = new Worker('./coordenador_fase4b.js');
...
log('MainHeap Fase 4b iniciado, ping enviado ao coordenador <==================================', true);
